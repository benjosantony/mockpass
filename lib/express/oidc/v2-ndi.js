  ...
  for (const idp of ['singPass', 'corpPass']) {
    ...
    app.post(
      ...
      async (req, res) => {
        ...
        console.log('Reached the token end point')
        ...
        const signedIdToken = await jose.JWS.createSign(
          { format: 'compact', alg: 'ES512' },
          signingKey,
        )
          .update(JSON.stringify(idTokenClaims))
          .final()

        console.log('signedIdToken', signedIdToken)
    ...
    app.get(
      ...
      (req, res) => {
        ...
        const data = {
          ...
          id_token_signing_alg_values_supported: ['ES256', 'ES512'],
